...
def preproceess_obs(obs, size):
    ...
    curr_obs["proprio"] = get_qpos(obs.robot_pose)
...
class EasoGymEnv(gym.Env):
        ...
        self.observation_space = gym.spaces.Dict(
            {
                ...
                "proprio": gym.spaces.Box(
                    low=np.ones((9,)) * -1, high=np.ones((9,)), dtype=np.float64
                ),
            }
        )
        self.action_space = gym.spaces.Box(
            low=np.zeros((10,)), high=np.ones((10,)), dtype=np.float64
        )
        
        self._env = XarmMDP_DeltaPosAbsRot(control_freq=50)    
    ...
    def step(self, action):      
        ...
        obs = self._env.step(action, None)
        obs = preproceess_obs(obs, self.im_size)
        obs['timestep'] = np.array([self.time_step])
    ...
    def pre_position(self):
        ...
        while not done:
            ...
            obs, info = self._env.step(action_infos, t)
